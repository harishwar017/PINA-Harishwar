# -*- coding: utf-8 -*-
"""
Created on Sat Mar 18 15:43:02 2023

@author: rahul
"""

import scipy.io
import matplotlib.pyplot as plt

method = 'AD'

Data_used = 1000


# Ground Truth
u_actual = scipy.io.loadmat('../Results.mat')['u_Mat']
time_actual = scipy.io.loadmat('../Results.mat')['total_time'].T
x_actual = scipy.io.loadmat('../Results.mat')['total_L']

# Prediction

u_pred = scipy.io.loadmat('../Burgers_Dis_{}_{}.mat'.format(method,Data_used))['predicted_output_{}_{}'.format(method,Data_used)].reshape(100,20).T
x_pred = scipy.io.loadmat('../Burgers_Dis_{}_{}.mat'.format(method,Data_used))['pts_array_{}_{}'.format(method,Data_used)][:,0].reshape(100,20).T
time_pred = scipy.io.loadmat('../Burgers_Dis_{}_{}.mat'.format(method,Data_used))['pts_array_{}_{}'.format(method,Data_used)][:,1].reshape(100,20).T




fig, ax = plt.subplots(nrows=1, ncols=3, figsize=(24, 8))

#plot Original 

custom_xticks = [0.2, 0.4, 0.6, 0.8]  # Replace with your desired custom tick values

cb = getattr(ax[0], 'contourf')(x_actual,time_actual,u_actual)
colorbar =fig.colorbar(cb,ax = ax[0])
colorbar.ax.tick_params(labelsize=20)
ax[0].set_title("Ground Truth",fontsize=28)
ax[0].set_xlabel('x',fontsize=28)
ax[0].set_ylabel('t',fontsize=28)
ax[0].set_xticks(custom_xticks)


#plot prediciton

cb = getattr(ax[1], 'contourf')(x_pred,time_pred,u_pred)
colorbar =fig.colorbar(cb,ax = ax[1])
colorbar.ax.tick_params(labelsize=20)
ax[1].set_title("Prediction",fontsize=28)
ax[1].set_xlabel('x',fontsize=28)
ax[1].set_ylabel('t',fontsize=28)
ax[1].set_xticks(custom_xticks)


#plot absolute error

cb = getattr(ax[2], 'contourf')(x_pred,time_pred,abs(u_pred-u_actual))
colorbar =fig.colorbar(cb,ax = ax[2])
colorbar.ax.tick_params(labelsize=20)
ax[2].set_title("error (AE)",fontsize=28)
ax[2].set_xlabel('x',fontsize=28)
ax[2].set_ylabel('t',fontsize=28)
ax[2].set_xticks(custom_xticks)



# Adjust x-axis and y-axis tick label font sizes
for i in range(3):
    ax[i].tick_params(axis='x', labelsize=24)
    ax[i].tick_params(axis='y', labelsize=24)

plt.savefig('Plots_comparison_2D/{}_{}.pdf'.format(method, Data_used), bbox_inches = 'tight', dpi=1200)


# a = abs(u_pred-u_actual)

# a_plain = a.reshape(-1,1)

# a_max = max(a_plain)




            
# cb = getattr(ax[1], method)(*grids, truth_output.cpu().detach(), **kwargs)
#             fig.colorbar(cb, ax=ax[1])
# cb = getattr(ax[2], method)(*grids,
#                                         (truth_output-pred_output).cpu().detach(),
#                                         **kwargs)
#             fig.colorbar(cb, ax=ax[2])


...
def advection(output_, input_, velocity_field, components=None, d=None):
    ...
    # print(f"temp shape: {tmp.shape}")
    # print(f"velocity shape: {velocity_field.shape}")

    tmp *= velocity_field
    return tmp.sum(dim=2).T


def curl(output_, input_, components=None, d=None):
    if d is None:
        d = input_.labels
    ...
    if len(components) > 2:
        raise ValueError("Components must be less than or equal to 2")

    grad_output = grad(output_, input_, components, d)
    result = torch.zeros(output_.shape[0], 2, device=output_.device)
    # print(result.shape)
    labels = [None] * len(d)
    
    if len(components) == 1:
        # Assuming components[0] is the stream function psi in 2D
        c_field_y = f'd{components[0]}d{d[1]}'
        c_field_x = f'd{components[0]}d{d[0]}'
        
        u = grad_output.extract([c_field_y])
        # print(u.shape)
        v = -grad_output.extract([c_field_x])
    ...
    if len(components) == 2:
        ...
        # 2D curl is a scalar
        c_fields1 = f'd{components[1]}d{d[0]}'
        c_fields2 = f'd{components[0]}d{d[1]}'
        output = grad_output.extract([c_fields1]) - grad_output.extract([c_fields2])
        return output
